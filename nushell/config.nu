...
let dark_theme = {
    ...
    block: white
    hints: dark_gray
    search_result: {bg: black fg: white}
    ...
    shape_garbage: { fg: black bg: red attr: b}
...
$env.config = {
  ...
  explore: {
    ...
    config: {
      cursor_color: {bg: 'white' fg: 'black' }
  ...
  keybindings: [
...

    {
      ...
      event: { 
        ...
        cmd: 'commandline edit -i (history | reverse | get command | str join "\n" | menu)'
      }
    }

    {
      ...
      event: { 
        send: executehostcommand
        cmd: 'let res = fzf; if $res != "" { commandline edit -i $"($res) " }'
      }
    }

    {
      ...
      event: {
        send: executehostcommand
        cmd: '$env.FZF_DEFAULT_COMMAND = "fd --follow --type d"; let res = fzf; if $res != "" { cd $res } # commandline edit -i $"($res) " } $env.FZF_DEFAULT_COMMAND = ""'
      }
    }

    {
      name: yank_commandline
      modifier: control
      keycode: char_y
      mode: [emacs, vi_normal, vi_insert]
      event: {
